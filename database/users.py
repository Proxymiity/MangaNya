...
def create(username, password, email, private=False, state=0, admin=False, uploader=False):
    dbc.execute("insert into users (username, password, email, private, state, admin, uploader) "
                "values (%s, %s, %s, %s, %s, %s, %s) returning *",
                (username, password, email, private, state, admin, uploader))
    db.commit()
    w = dbc.fetchall()[0]
...
def update(id_, username, password, email, private, state, admin, uploader):
    dbc.execute("update users set username = %s, password = %s, email = %s, private = %s, state = %s, "
                "admin = %s, uploader = %s, updated_at = (now() at time zone 'utc') "
                "where id = %s",
                (username, password, email, private, state, admin, uploader, id_))
    db.commit()
